...
void us_ticker_init(void)
{
    ...
    MBED_ASSERT((prescale_timer != (uint32_t) -1) && prescale_timer <= 255);
    ...
    timer_base->TCSR = (timer_base->TCSR & ~TIMER_TCSR_PRESCALE_Msk) | TIMER_CONTINUOUS_MODE | prescale_timer;
    timer_base->TCMPR = cmp_timer;
    ...
    while(!(timer_base->TCSR & TIMER_TCSR_CACT_Msk));
}
