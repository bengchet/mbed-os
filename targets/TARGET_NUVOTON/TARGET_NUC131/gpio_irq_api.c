...
static struct nu_gpio_irq_var gpio_irq_var_arr[] = {
    {NULL, 0, GPAB_IRQHandler},
    {NULL, 1, GPAB_IRQHandler},
    {NULL, 2, GPCDEF_IRQHandler},
    {NULL, 3, GPCDEF_IRQHandler},
    {NULL, 4, GPCDEF_IRQHandler},
    {NULL, 5, GPCDEF_IRQHandler}
};
...
#define MBED_CONF_TARGET_GPIO_IRQ_DEBOUNCE_CLOCK_SOURCE GPIO_DBCLKSRC_LIRC
#endif
...
void gpio_irq_free(gpio_irq_t *obj)
{
    ...
    NVIC_DisableIRQ((var->gpio_n < 2) ? GPAB_IRQn : GPCDEF_IRQn);
    NU_PORT_BASE(port_index)->IEN = 0;
    ...
    switch (event) {
        case IRQ_RISE:
            if (enable) {
                ...
                gpio_base->IEN &= ~(GPIO_INT_RISING << pin_index);
            }
            break;
        ...
        case IRQ_FALL:
            if (enable) {
                ...
                gpio_base->IEN &= ~(GPIO_INT_FALLING << pin_index);
            }
            break;
...
void gpio_irq_enable(gpio_irq_t *obj)
{
    ...
    NVIC_SetVector((var->gpio_n < 2) ? GPAB_IRQn : GPCDEF_IRQn, (uint32_t) var->vec);
    NVIC_EnableIRQ((var->gpio_n < 2) ? GPAB_IRQn : GPCDEF_IRQn);
}
